...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'NSF Awards');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'get_award':
      return getAward(reqStr(args, 'award_id', '"2125634" (numeric NSF award ID, passed as string)'));
    default:
...
async function searchAwards(args: Record<string, unknown>) {
  ...
  const res = await pwFetch(`${BASE_URL}.json?${params}`);
  if (!res.ok) throw await httpError(res, 'NSF API error');
